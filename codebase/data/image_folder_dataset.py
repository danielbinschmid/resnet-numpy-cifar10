...
class ImageFolderDataset(Dataset):
    ...
    def __init__(self, *args, transform=None, mode='train', data_zip_file: str = "./data/cifar10.zip" , limit_files=None,
                 split={'train': 0.6, 'val': 0.2, 'test': 0.2},
                 **kwargs):
        super().__init__(*args, 
                         download_url=None,
                         **kwargs)
        extract_archive(data_zip_file, remove_finished=False)
...
class ReshapeTransform:
    ...
    def __call__(self, images: np.ndarray) -> any:
        reshape_order = 'F'
        if len(images.shape) == 4:
            ...
            images = images.reshape((nImgs, nChannels, nW, nH), order=reshape_order)
        else:
            nW = images.shape[0]
            nH = images.shape[1]
            nChannels = images.shape[2]
            images = images.reshape((nChannels, nW, nH), order=reshape_order)
        return images
