...
class conv_layer:
    ...
    def __init__(self, in_channels, out_channels, kernel_h, kernel_w, same = True, stride = 1, shift = True, id="conv"):
        self.in_channels = in_channels
        self.out_channels = out_channels
    ...
    def get_params(self, params):
        params[self.id + "_kernel"] = self.kernel

    def set_params(self, params):
        self.kernel = params[self.id + "_kernel"]
    ...
    def convolution(in_tensor, kernel, stride = 1, dilate = 1):
        """
        in_tensor shape: (batch_num, in_channels, in_h, in_w
        )"""
        batch_num = in_tensor.shape[0]
        in_channels = in_tensor.shape[1]
        in_h = in_tensor.shape[2]
    ...
    def forward(self, in_tensor):
        if self.same:
            ...
            # in_tensor = conv_layer.pad(in_tensor, int((self.kernel_h)/2) - 1, int((self.kernel_w)/2) - 1) 
    ...
    def backward(self, out_diff_tensor, grads):
        assert out_diff_tensor.shape == self.out_tensor.shape
        ...
        grads[self.id + "_kernel"] = kernel_diff
...
class global_max_pooling:
    
    def forward(self, in_tensor):
        self.shape = in_tensor.shape
        out_tensor = in_tensor.reshape(in_tensor.shape[0], in_tensor.shape[1], -1).max(axis = -1)
...
class bn_layer:
    ...
    def __init__(self, neural_num, moving_rate = 0.1, id="batch_norm"):
    ...
    def get_params(self, params):
        params[self.id + "_gamma"] = self.gamma
        params[self.id + "_bias"] = self.bias
        return params
    
    def set_params(self, params):
        self.gamma = params[self.id + "_gamma"]
        self.bias = params[self.id + "_bias"]
    ...
    def backward(self, out_diff_tensor, grads):
        ...
        grads[self.id + "_gamma"] = gamma_diff
        grads[self.id + "_bias"] = bias_diff
        return self.in_diff_tensor
